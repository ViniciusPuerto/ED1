...
//Montando lista das Industrias:
    ...
    parq = fopen("/Users/niltonsilva/OneDrive/Sync/Nilton/UnB/2017_1/Disciplinas/EDA/Aulas/17_Atividade2/Publicacao/Industria.txt", "r");
    if(parq==NULL)
        return NULL;
...
TNoC *CriaListaComercio()
{
    ...
    parq = fopen("/Users/niltonsilva/OneDrive/Sync/Nilton/UnB/2017_1/Disciplinas/EDA/Aulas/17_Atividade2/Publicacao/Comercio.txt", "r");
    if(parq==NULL)
        return NULL;
...
int CriaListaServico(TDescritorS *pLSer)
{
    ...
    parq = fopen("/Users/niltonsilva/OneDrive/Sync/Nilton/UnB/2017_1/Disciplinas/EDA/Aulas/17_Atividade2/Publicacao/Servico.txt", "r");
    if(parq==NULL)
        return 0;
...
{
    FILE* parqI, * parqC, * parqS;
    TCadastroU pregauxI, pregauxC, pregauxS ;
    TNoU *plista, *pnovonoI, *pnovonoC, *pnovonoS, *pnovono;
    TDescritorU *pdescritor;
    // Bloco para leitura dos arquivos

    // Trocar caminho dos dados aqui
    parqI = fopen("./Users/niltonsilva/OneDrive/Sync/Nilton/UnB/2017_1/Disciplinas/EDA/Aulas/17_Atividade2/Publicacao/Industria.txt", "r");
    if(parqI==NULL)
        

    parqC = fopen("./Users/niltonsilva/OneDrive/Sync/Nilton/UnB/2017_1/Disciplinas/EDA/Aulas/17_Atividade2/Publicacao/Comercio.txt", "r");
    if(parqC==NULL)
        return NULL;

    parqS = fopen("./Users/niltonsilva/OneDrive/Sync/Nilton/UnB/2017_1/Disciplinas/EDA/Aulas/17_Atividade2/Publicacao/Servico.txt", "r");
    if(parqS==NULL)
        return NULL;
    
    // Bloco para carregamento da lista
    plista = NULL;
    // Lê os arquivos para a lista
    while(fscanf(parqS,"%s %s %s %s", pregauxS.CNPJ, pregauxS.RazaoSocial, pregauxS.Cidade, pregauxS.Fone)>0 && 
        fscanf(parqI,"%s %s %s %s", pregauxI.CNPJ, pregauxI.RazaoSocial, pregauxI.Cidade, pregauxI.Fone)>0 && 
            fscanf(parqC,"%s %s %s %s", pregauxC.CNPJ, pregauxC.RazaoSocial, pregauxC.Cidade, pregauxC.Fone)>0) {
                
                // Nos auxiliares para serem concatenados na nova lista unificada
                pnovonoI = (TNoU *)malloc(sizeof(TNoU));
                pnovonoC = (TNoU *)malloc(sizeof(TNoU));
                pnovonoS = (TNoU *)malloc(sizeof(TNoU));
                pnovonoI->Dados = (TCadastroU *)malloc(sizeof(TCadastroU));
                pnovonoC->Dados = (TCadastroU *)malloc(sizeof(TCadastroU));
                pnovonoS->Dados = (TCadastroU *)malloc(sizeof(TCadastroU));

                strcpy(pnovonoI->Dados->CNPJ, pregauxI.CNPJ);
                strcpy(pnovonoI->Dados->RazaoSocial, pregauxI.RazaoSocial);
                strcpy(pnovonoI->Dados->Cidade, pregauxI.Cidade);
                strcpy(pnovonoI->Dados->Fone, pregauxI.Fone);
                pnovonoI->Dados->Tipo = 1;

                strcpy(pnovonoC->Dados->CNPJ, pregauxC.CNPJ);
                strcpy(pnovonoC->Dados->RazaoSocial, pregauxC.RazaoSocial);
                strcpy(pnovonoC->Dados->Cidade, pregauxC.Cidade);
                strcpy(pnovonoC->Dados->Fone, pregauxC.Fone);
                pnovonoC->Dados->Tipo = 2;

                strcpy(pnovonoS->Dados->CNPJ, pregauxS.CNPJ);
                strcpy(pnovonoS->Dados->RazaoSocial, pregauxS.RazaoSocial);
                strcpy(pnovonoS->Dados->Cidade, pregauxS.Cidade);
                strcpy(pnovonoS->Dados->Fone, pregauxS.Fone);
                pnovonoS->Dados->Tipo = 3;
                
                // Concatencação dos nos
                pnovono->Prox = pnovonoI;
                pnovonoI->Prox = pnovonoC;
                pnovonoS->Prox = pnovono;

                pnovono->Ant  = NULL;
                pnovono->Prox = pListaUnica->Inicio;

                if(pListaUnica->Tamanho > 0)
                    pListaUnica->Inicio->Ant = pnovono;
                else {
                    pListaUnica->Fim = pnovono;
                }
                pListaUnica->Inicio = pnovono;
                pListaUnica->Tamanho++;
            }
